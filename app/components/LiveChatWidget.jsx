...
export default function LiveChatWidget({ restaurantId = 'global-admin-chat' }) {
  ...
  useEffect(() => {
    ...
    // Realtime subscription for incoming admin replies
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      {!isOpen ? (
        ...
        >
          <span>💬</span>
          <span className="text-xs uppercase tracking-wider pr-1">Live Chat</span>
        </button>
      ) : (
        <div className="bg-neutral-900 border border-neutral-800 rounded-3xl w-80 sm:w-96 h-[450px] shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="bg-neutral-950 p-4 border-b border-neutral-800 flex justify-between items-center">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-black text-white uppercase tracking-wider">Digital Dining Support</h3>
            </div>
            <button
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-neutral-950/50">
            {messages.length === 0 ? (
              <p className="text-center text-xs text-neutral-500 mt-12">Ask us anything! We are online to help.</p>
            ) : (
              messages.map((msg) => (
                <div
                  key={msg.id}
          ...
          {/* Input Footer */}
          <form onSubmit={sendMessage} className="p-3 bg-neutral-950 border-t border-neutral-800 flex space-x-2">
            <input
              type="text"
