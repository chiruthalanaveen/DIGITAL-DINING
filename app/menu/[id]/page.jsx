...
// The uploaded restaurant logo is always preferred over the Digital Dining branding.
...
export default function CustomerMenuPage() {
  ...
  const fetchMenu = async () => {
    ...
    try {
      ...
      // Customers only see active offers scheduled for the current day.
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(
        `customer-menu-sync-${restaurantId}`
      )
      .on(
        'postgres_changes',
        {
  ...
  const sgstRate = Number(
    restaurant?.sgst_rate ?? 2.5
  )

  const cgstRate = Number(
    restaurant?.cgst_rate ?? 2.5
  )
  ...
  const shouldShowHighlyReordered =
    item => {
      const mode =
        item?.reorder_mode ||
        'auto'
      ...
      return automaticHighlyReorderedIds.has(
        item?.id
      )
    }
  ...
  const handlePayAtCounter =
    async () => {
      ...
      if (!counterPaymentEnabled) {
        alert(
          'Pay at Counter is currently disabled by this restaurant.'
        )
  ...
  const handleRazorpayCheckout =
    async () => {
      ...
      const activeKeyId =
        restaurant?.razorpay_key_id?.trim()
      ...
      try {
        const options = {
          ...
          description:
            `Table ${tableNumber} Order (${customerName})`,
          ...
          handler:
            async response => {
              try {
                ...
                console.error(
                  'Online order logging error:',
                  error
                )

                alert(
                  'Payment received, but failed to log order: ' +
                    error.message
                )
              } finally {
                setPaymentLoading(
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center text-neutral-800 gap-3">
        <div className="w-10 h-10 border-4 border-neutral-200 border-t-red-500 rounded-full animate-spin" />
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                autoComplete="off"
                required
                className="w-full px-4 py-3.5 rounded-2xl bg-neutral-50 border border-neutral-200 focus:border-red-400 focus:outline-none text-sm font-medium !text-black placeholder:!text-neutral-400 caret-black" style={{ color: "#000000", opacity: 1 }}
              />
            </div>
          ...
          <div className="mt-4 rounded-2xl border border-neutral-200 p-4">
            ...
            <div className="border-t border-neutral-100 mt-4 pt-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {money(
                    confirmedBillSummary.gstAmount /
                      2
                  )}
                </span>
              </div>
              ...
              {confirmedBillSummary.packingFee >
                0 && (
                <div className="flex justify-between">
                  <span className="text-neutral-500">
                    Packing
  ...
  return (
    <>
      <style jsx global>{`
        /* Keep all customer-entered text readable */
        ...
        /* Restaurant name visibility fix — never allow the name to appear transparent. */
        .dd-restaurant-name {
          color: #000000 !important;
          -webkit-text-fill-color: #000000 !important;
          opacity: 1 !important;
        ...
        /* Premium QR-menu polish — visual only, existing functionality untouched */
        ...
        .dd-glow-btn:active { transform: scale(.97); }
        .dd-floating { animation: dd-float 3.2s ease-in-out infinite; }
        .dd-pulse { animation: dd-pulse-soft 2.2s infinite; }
        .dd-chip {
        ...
        <div className="max-w-md mx-auto relative h-52 rounded-[26px] overflow-hidden bg-gradient-to-br from-orange-400 via-red-500 to-red-600 shadow-xl ring-1 ring-black/5">
          ...
          {restaurant.banner_url ? (
            ...
            <>
              ...
              <div className="absolute right-4 bottom-3 text-7xl">
                🍽️
              </div>
            </>
          )}
      ...
      {/* VISUAL TRUST CHIPS — display only, no feature/logic changes */}
      <section className="max-w-md mx-auto px-4 mt-4">
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          <div className="dd-chip shrink-0 rounded-full px-3 py-2 flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-neutral-700">Freshly prepared</span>
          </div>
          <div className="dd-chip shrink-0 rounded-full px-3 py-2 flex items-center gap-2">
            <span>⚡</span>
            <span className="text-[10px] font-black text-neutral-700">Quick ordering</span>
          </div>
          <div className="dd-chip shrink-0 rounded-full px-3 py-2 flex items-center gap-2">
            <span>🛡️</span>
            <span className="text-[10px] font-black text-neutral-700">Secure checkout</span>
          </div>
        </div>
      </section>

      {/* OFFERS OF THE DAY — managed only from the restaurant dashboard */}
        ...
        <section className="max-w-md mx-auto px-4 mt-5">
          <div className="rounded-[26px] overflow-hidden bg-gradient-to-br from-red-600 via-orange-500 to-amber-400 shadow-xl ring-1 ring-red-200/60">
            <div className="p-4 flex items-center justify-between">
              <div>
                <p className="text-[9px] font-black uppercase tracking-[0.22em] text-white/80">Limited time today</p>
                <h2 className="text-xl font-black text-white mt-1">🔥 Offers of the Day</h2>
              </div>
              <div className="bg-white/20 backdrop-blur-md rounded-full px-3 py-1.5 text-[9px] font-black text-white border border-white/20">TODAY ONLY</div>
            </div>
            <div className="px-3 pb-3 space-y-3">
              {dailyOffers.map((offer) => (
                <div key={offer.id} className="bg-white rounded-[22px] p-3 flex gap-3 items-center shadow-lg">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-black text-neutral-900 text-sm leading-tight">{offer.title}</h3>
                      {offer.discount_text && <span className="shrink-0 bg-red-100 text-red-600 px-2 py-1 rounded-full text-[9px] font-black">{offer.discount_text}</span>}
                    </div>
                    {offer.description && <p className="text-[10px] text-neutral-500 mt-1 line-clamp-2">{offer.description}</p>}
                    <div className="flex items-end gap-2 mt-2">
                      {offer.original_price != null && Number(offer.original_price) > 0 && <span className="text-[11px] text-neutral-400 line-through">₹{Number(offer.original_price).toFixed(2)}</span>}
                      {offer.offer_price != null && <span className="text-lg font-black text-red-600">₹{Number(offer.offer_price).toFixed(2)}</span>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="flex gap-2">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="dd-search w-full h-14 rounded-2xl border border-neutral-200 bg-white shadow-sm pl-12 pr-12 text-sm font-medium focus:outline-none focus:border-red-400 !text-black placeholder:!text-neutral-400 caret-black" style={{ color: "#000000", opacity: 1 }}
            />
