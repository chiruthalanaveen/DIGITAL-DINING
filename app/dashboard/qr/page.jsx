...
export default function TableQRPage() {
  ...
  useEffect(() => {
    async function fetchRestaurantQRData() {
      ...
      if (error) {
        console.error('Restaurant QR data error:', error)
      }
  ...
  const qrCodeImageUrl =
    `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(menuUrl)}`
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6 font-sans">
      <div className="max-w-xl mx-auto space-y-6">
        ...
        {/* Header Bar */}
        <div className="flex justify-between items-center border-b border-neutral-800 pb-4 gap-4">
          ...
          <button
            ...
            className="shrink-0 bg-neutral-800 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-neutral-700 transition border border-neutral-700"
          >
            ← Back to Dashboard
          </button>
        </div>
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl space-y-6 shadow-2xl text-center">
          ...
          <div className="space-y-2 text-left">
            <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-wider text-center">
              Unique Public Menu Link for Table {tableNumber}:
            </p>
